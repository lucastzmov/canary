...
function talk.onSay(player, words, param)
  ...
  if not canOpen then
    player:sendTextMessage(MESSAGE_FAILURE, "Espere " .. math.ceil(waitTime) .. " segundos antes de abrir novamente.")
    return true
  end
...
-- local function hasAddon(player, storages)
--   for _, storage in ipairs(storages) do
--     if player:getStorageValue(storage) < 1 then
--       return false -- Se algum dos storages ainda não está ativado, não tem o addon completo
--     end
--   end
--   return true
-- end

-- local function onSelected(player, button, choice)
--   if button == 2 then -- Fechar
--     return true
--   end

--   if type(choice) == "table" then
--     choice = choice.text or ""
--   end

--   local addonData = addons[choice]
--   if not addonData then
--     player:sendTextMessage(MESSAGE_FAILURE, "Addon inválido.")
--     return true
--   end

--   if not addonData.storages or type(addonData.storages) ~= "table" then
--     print("[ERRO] Storages não definidos corretamente para addon:", choice)
--     player:sendTextMessage(MESSAGE_FAILURE, "Erro interno: storages ausentes.")
--     return true
--   end

--   if hasAddon(player, addonData.storages) then
--     player:sendTextMessage(MESSAGE_FAILURE, "Você já possui o addon '" .. choice .. "'.")
--     return true
--   end

--   if not player:removeMoney(PRICE) then
--     player:sendTextMessage(MESSAGE_FAILURE, "Você precisa de " .. PRICE .. " gold coins.")
--     return true
--   end

--   player:addOutfitAddon(addonData.outfit_female, addonData.addons)
--   player:addOutfitAddon(addonData.outfit_male, addonData.addons)

--   -- Zera as storages de missões (se existir)
--   if addonData.storagesSetZero then
--     for _, storage in ipairs(addonData.storagesSetZero) do
--       player:setStorageValue(storage, 0)
--     end
--   end

--   -- Marca todos os storages do addon como recebidos
--   for _, storage in ipairs(addonData.storages) do
--     player:setStorageValue(storage, addonData.addons)
--   end

--   player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Você comprou o addon: " .. choice .. "!")
...
--     title = "Comprar Addons",
--     message = "Escolha um addon completo para comprar por " .. PRICE .. " gold coins:"
--   })

--   for name, data in pairs(addons) do
--     if not hasAddon(player, data.storage) then
--       modal:addChoice(name, onSelected, name)
--     end
--   end
